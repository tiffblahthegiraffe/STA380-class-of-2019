...
FXmonthly = read.csv(url('https://raw.githubusercontent.com/jgscott/STA380/master/data/FXmonthly.csv'), header=TRUE)
summary(FXmonthly)

# USD-GBP
plot(FXmonthly$exukus)

#the problem is in FX market, variables are highly correlated
...
cor(FXmonthly_ret[,c('exalus','exbzus','excaus','exchus','exdnus')])
...
fx_scores = predict(fxpca)  # same as fxpca$x (it's the score of Z)
...
# When the US dollar goes up, all the other FX goes down. Becuz US dollar is the
# dominant the FX market. This is called the 'dollar effect'

# Refer to the actual currency/country names
currency_codes = read.table(url('https://raw.githubusercontent.com/jgscott/STA380/master/data/currency_codes.txt'))
currency_codes
