...
gonefishing = read.csv(url('https://raw.githubusercontent.com/jgscott/STA380/master/data/gonefishing.csv'), header=TRUE)
summary(gonefishing)
...
n_fish = 30
fishing_trip = mosaic::sample(gonefishing, n_fish) #mosaic sample
...
# Using the more flexible "foreach" construction to repeat 25 times
...
# Simulate a whole year of 30-fish days (repeat 36500 times)
...
hist(my_fishing_year, 25) #this is not the hist from original data
sd(my_fishing_year) #the sd of those random sampling 
...
sd(my_fishing_year) #sample from the popultaion (repeat 36500 times)
sd(boot1) #boostrap (repeat 2500 times)
