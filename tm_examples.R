...
adam = readerPlain("https://raw.githubusercontent.com/jgscott/STA380/master/data/division_of_labor.txt")
adam
meta(adam)
content(adam)
...
file_list = Sys.glob('https://github.com/jgscott/STA380/tree/master/data/ReutersC50/C50train/SimonCowell/*.txt')
simon = lapply(file_list, readerPlain) #apply func in every item in list
...
stopwords("en") #stopword in english
stopwords("SMART") 
?stopwords
...
which(clust5 == 1)
content(simon[[1]])
content(simon[[16]]) #see some contents in the high similarity documents
content(simon[[13]])
